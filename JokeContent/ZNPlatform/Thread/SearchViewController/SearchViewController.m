...
- (AJSearchBar *)searchBar{
    if (!_searchBar) {
        _searchBar = [[AJSearchBar alloc]initWithFrame:CGRectMake(zn_AutoWidth(60),znStateHeight, screenWidth - zn_AutoWidth(50),[RPConfig sharedInstance].navigationHeight)];
...
- (void)updataFrame{
    row = [_searchVM rowForCollection:[_searchVM readHistory]];
    _searchView.frame = CGRectMake(0, zn_AutoHeight(110), screenWidth, (row * 45)+[RPConfig sharedInstance].tabBarHeight);
}
